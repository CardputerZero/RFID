...
public:
    ...
    bool save_emu_dump_cached(ProtocolKind protocol, int slot, std::string *err = nullptr)
    {
        ...
        // Build a SavedRecord and save emulator JSON to dumps dir
        SavedRecord record;
        record.meta.created_at     = iso8601_now();
        std::string stem;
        switch (protocol) {
        case ProtocolKind::Iso14443A: stem = "ntag213"; break;
        case ProtocolKind::MifareClassic: stem = "mfc1k"; break;
        case ProtocolKind::Iso15693: stem = "iso15693"; break;
        default: stem = "unknown"; break;
        }
        record.meta.record_id      = stem;
        ...
        record.tag = tag;
        storage_.save_emulator_json_from_record(protocol, record);
        // Also save as a standard record so it appears in SAVED list
